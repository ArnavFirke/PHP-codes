...
<form action="{{Request::segment(2)=='add'?route('booking.save'):route('booking.update',['id'=>Request::segment(2)])}}" method="POST">
  ...
  <div class="container">
      ...
      <div class="mb-3 w-50">
          ...
          <input type="date" class="form-control" id="booking_on" name="booking_on" placeholder="Booking Date" value="{{ isset($booking->booking_datetime) ? \Carbon\Carbon::parse($booking->booking_datetime)->format('Y-m-d') : '' }}">
      </div>
